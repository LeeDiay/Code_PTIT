...
public class PlayOverlay {
	...
	public void mousePressed(MouseEvent e) {
		...
		if (isIn(e, menu_button) && gp.gameState == gp.playState) {
			menu_button.setMousePressed(true);
			gp.gameState = gp.pauseState;
		}
		
		else if (isIn(e, reset_button) && gp.gameState == gp.playState) {
			reset_button.setMousePressed(true);
			gp.setupGameWithoutPause();
		}
		
		else if (isIn(e, ins_button) && gp.gameState == gp.playState) {
			ins_button.setMousePressed(true);
			gp.popUp = true;
	...
	public void mouseMoved(MouseEvent e) {
			...
			if (isIn(e, reset_button ) && gp.gameState == gp.playState) {
				gp.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));	
			}
			
			if (isIn(e, ins_button ) && gp.gameState == gp.playState) {
				gp.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));	
			}
